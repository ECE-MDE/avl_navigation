...
#ifndef SINS_MUKF_DVL_H
#define SINS_MUKF_DVL_H
...
struct NavStateDvl
{
    ...
    Vector3d b_v =    Vector3d::Zero();
    Vector3d S_v =    Vector3d::Zero();
    ...
    VectorXd to_vector()
    {
        ...
        VectorXd x(21);
        x << theta_n_b, v_eb_b, p_b, b_g, b_a, b_v, S_v;
        return x;
    }
...
class SinsMukfDvl : public NavFilter
{
...
public:
    ...
    //              - dt: Time step in seconds.
    //------------------------------------------------------------------------------
    void iterate(Vector3d w_ib_b, Vector3d f_ib_b, double dt, double T_c_g, double T_c_a, double T_c_v);
...
public:
    ...
    static NavStateDvl se23_to_state(MatrixXd M);
    static MatrixXd state_to_se23 (NavStateDvl state);
    ...
    // Returns:     ???.
    //--------------------------------------------------------------------------
    static NavStateDvl phi_func(NavStateDvl x, VectorXd xi);
    ...
    // Returns:     ???.
    //--------------------------------------------------------------------------
    static VectorXd phi_inv_func(NavStateDvl x, NavStateDvl x_hat);
    ...
    // Returns:     New state.
    //--------------------------------------------------------------------------
    static NavStateDvl f_func(NavStateDvl x, NavInputDvl omega, VectorXd w);
    ...
    //--------------------------------------------------------------------------
    static VectorXd h_func_pos(NavStateDvl x, Vector3d l_bS_b);
    ...
    //              state.
    //--------------------------------------------------------------------------
    static VectorXd h_func_vel(NavStateDvl x, Vector3d l_bS_b, Vector3d w_ib_b);
    ...
    //--------------------------------------------------------------------------
    static VectorXd h_func_depth(NavStateDvl x, double alt_surface, Vector3d l_bS_b);
    ...
    //--------------------------------------------------------------------------
    static VectorXd h_func_range(NavStateDvl x, Vector3d p_source, Vector3d l_bS_b);
    ...
    // Returns:     Position measurement corresponding to current state.
    //--------------------------------------------------------------------------
    static VectorXd h_func_gps(NavStateDvl x, Vector3d l_bS_b, Vector3d w_ib_b);

private:

    // MUKF instance
    Mukf<NavStateDvl, NavInputDvl> mukf;
